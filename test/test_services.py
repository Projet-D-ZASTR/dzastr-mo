...
def test_creer_service(client):
    res = client.post("/services/", json={"nom": "Dev web", "prix_heure": 75.0})
    assert res.status_code == 201
    data = res.json()
    assert data["service_nom"] == "Dev web"
    assert float(data["service_prixHeure"]) == 75.0
...
def test_obtenir_service(client):
    ...
    assert res.status_code == 200
    assert res.json()["service_nom"] == "Dev web"
...
def test_modifier_service_nom(client):
    ...
    assert res.status_code == 200
    assert res.json()["service_nom"] == "Dev mobile"
    assert float(res.json()["service_prixHeure"]) == 75.0
...
def test_modifier_service_prix(client):
    ...
    assert res.status_code == 200
    assert float(res.json()["service_prixHeure"]) == 90.0
